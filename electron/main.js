...
ipcMain.handle("stem-model-ensure", async (evt) => {
  try{
    ...
    try{ fs.rmSync(path.join(app.getPath("userData"), "stem-roformer"), { recursive:true, force:true }); }catch(e){}
...
ipcMain.handle("export-thumb", async (evt, args) => {
  ...
  try{
    ...
    if(args.pm) q += "&pm=1";   // exporta lo que se está previsualizando (miembro), no la portada propia
